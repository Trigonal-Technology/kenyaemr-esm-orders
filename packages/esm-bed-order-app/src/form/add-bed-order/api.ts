...
export interface BedOrderPost extends OrderPost {
  quantity?: number;
...
export function createPrepBedOrderPostData(
  bedOrderTypeUuid: string,
  careSettingUuid: string,
): PostDataPrepBedOrderFunction {
  return (order: BedOrderBasketItem, patientUuid: string, encounterUuid: string) => {
    return prepBedOrderPostData(order, patientUuid, encounterUuid, bedOrderTypeUuid, careSettingUuid);
  };
}

export function prepBedOrderPostData(
  ...
  if (order.action === 'NEW' || order.action === 'RENEW') {
    payload = {
      ...
      ward: order.ward,
      admissionReason: order.admissionReason,
    };
    return payload;
...
export type PostDataPrepBedOrderFunction = (
  order: BedOrderBasketItem,
  patientUuid: string,
  encounterUuid: string,
) => OrderPost;
